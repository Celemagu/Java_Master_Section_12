...
public class EjemploArreglosDetectarOrden {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i <a.length;i++) {
			a[i]= s.nextInt();// rellenamos el arreglo con lo que ingrese en consola
		}
		...
		for (int i = 0; i < a.length -1; i++) {
			...
			if (ascendente == true && descendente == false) {
				System.out.println("Arreglo = Odenado de forma Ascendente");
			}
			
			if (ascendente == false && descendente == true) {
				System.out.println("Arreglo = Odenado de forma Descendente");
			}
